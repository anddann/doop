...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      for (int i = 0; i < args.length; i++) {
        switch (args[i]) {
          ...
          case "-lsystem":
            ...
            sootParameters._libraries.add(
                javaHome + File.separator + "lib" + File.separator + "rt.jar");
            sootParameters._libraries.add(
                javaHome + File.separator + "lib" + File.separator + "jce.jar");
            sootParameters._libraries.add(
                javaHome + File.separator + "lib" + File.separator + "jsse.jar");
            break;
          case "--deps":
            i = shift(args, i);
          ...
          case "-help":
            ...
            System.err.println(
                "  --main <class>                        Specify the main name of the main class");
            System.err.println(
                "  --ssa                                 Generate SSA facts, enabling flow-sensitive analysis");
            System.err.println(
                "  --full                                Generate facts by full transitive resolution");
            System.err.println(
                "  -d <directory>                        Specify where to generate csv fact files");
            System.err.println(
                "  -l <archive>                          Find classes in jar/zip archive");
            System.err.println(
                "  -lsystem                              Find classes in default system classes");
            System.err.println(
                "  --deps <directory>                    Add jars in this directory to the class lookup path");
            System.err.println(
                "  --only-application-classes-fact-gen   Generate facts only for application classes");
            System.err.println(
                "  --noFacts                             Don't generate facts (just empty files -- used for debugging)");
            System.err.println(
                "  --uniqueFacts                         Eliminate redundancy from facts");

            System.err.println(
                "  --generate-jimple                     Generate Jimple/Shimple files instead of facts");
            System.err.println(
                "  --generate-jimple-help                Show help information regarding bytecode2jimple");
            System.err.println("  --modulemode                          Analyze Java 9 modules");
            System.err.println(
                "  --modulename                          Name of the java 9 module to analyze");
          ...
          case "--generate-jimple-help":
            ...
            System.err.println("options:");
            System.err.println(
                "  --ssa                                 Generate Shimple files (use SSA for variables)");
            System.err.println(
                "  --full                                Generate Jimple/Shimple files by full transitive resolution");
            System.err.println(
                "  --stdout                              Write Jimple/Shimple to stdout");
            System.err.println(
                "  -d <directory>                        Specify where to generate files");
            System.err.println(
                "  -l <archive>                          Find classes in jar/zip archive");
            System.err.println(
                "  -lsystem                              Find classes in default system classes");
            System.err.println(
                "  --android-jars <archive>              The main android library jar (for android apks). The same jar should be provided in the -l option");
            throw new DoopErrorCodeException(0);
          default:
            if (args[i].charAt(0) == '-') {
      ...
      if (n != 0) System.err.println("Exiting with code " + n);
    } catch (Exception exc) {
      exc.printStackTrace();
    }
  }
  ...
  private static void produceFacts(SootParameters sootParameters) throws Exception {
    ...
    if (sootParameters._android) {
      ...
      try (JarInputStream jin =
              new JarInputStream(new FileInputStream(sootParameters._inputs.get(0)));
        ...
        while ((entry = jin.getNextJarEntry()) != null) {
          ...
          if (entry.getName().endsWith(".properties")) {
            Constructor constructor =
                FoundFile.class.getConstructor(new Class[] {String.class, String.class});
            constructor.setAccessible(true);
            FoundFile foundFile =
                (FoundFile) constructor.newInstance(sootParameters._inputs.get(0), entry.getName());
            propertyProvider.addProperties(foundFile);
            // FIXME: used reflection
            // propertyProvider.addProperties((new FoundFile(sootParameters._inputs.get(0),
            // entry.getName())));
          }
        }
      }
    ...
    if (sootParameters._moduleMode) {
      ...
      for (int i = 0; i < sootParameters._inputs.size(); i++) {
        ...
        Map<String, List<String>> map =
            ModulePathSourceLocator.v().getClassUnderModulePath(sootParameters._inputs.get(i));
        for (String module : map.keySet()) {
          ...
          SootModuleResolver.v()
              .resolveClass(
                  SootModuleInfo.MODULE_INFO,
                  SootClass.BODIES,
                  com.google.common.base.Optional.of(module));
        }
      }
    ...
    classes
        .stream()
        .filter((klass) -> isApplicationClass(sootParameters, klass))
        .forEachOrdered(SootClass::setApplicationClass);

    if (sootParameters._mode == SootParameters.Mode.FULL
        && !sootParameters._onlyApplicationClassesFactGen) {
    ...
    ThreadFactory factory =
        new ThreadFactory(writer, sootParameters._ssa, sootParameters._generateJimple);
    Driver driver = new Driver(factory, classes.size(), sootParameters._generateJimple);

    classes
        .stream()
        .filter(SootClass::isApplicationClass)
        .forEachOrdered(writer::writeApplicationClass);
  ...
  public static void retrieveAllSceneClassesBodies() {
    ...
    while (clIt.hasNext()) {
      ...
      if (cl.resolvingLevel() < SootClass.SIGNATURES) continue;
    ...
    if (executor.getException() != null) throw (RuntimeException) executor.getException();
  }
}
